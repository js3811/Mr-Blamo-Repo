"""
    Death Streams Context Menu Addon
...
addon = xbmcaddon.Addon('plugin.video.blamo')
...
def toggle_auto_play():
    if __autoplay_enabled():
        ...
        kodi.notify(msg='Death Streams Auto-Play Turned Off')
    else:
        addon.setSetting('auto-play', 'true')
        kodi.notify(msg='Death Streams Auto-Play Turned On')
...
def scraper_sort_order():
    ...
    runstring = 'RunAddon(plugin.video.blamo,%s)' % (kodi.get_plugin_url(queries))
    xbmc.executebuiltin(runstring)
...
def __is_blamo_listitem(li_path):
    ...
    try:
        lines = xbmcvfs.File(li_path).read()
        if lines and not lines.startswith('plugin://plugin.video.blamo'):
...
def main(argv=None):
    ...
    if __is_blamo_listitem(path):
        dialog = xbmcgui.Dialog()
        tools = __get_tools(path, media_type, title, year)
        try: ret = dialog.contextmenu([i[0] for i in tools])
        except: ret = dialog.select('Death Streams Tools', [i[0] for i in tools])
        if ret > -1:
            tools[ret][1](*tools[ret][2])
    else:
        kodi.notify(msg='Not a Death Streams Library Item')
